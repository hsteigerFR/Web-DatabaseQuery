  ...
  if($filename[1] == 'csv')
  {
   ...
   while($data = fgetcsv($handle))
   {
	if ($i>0) {
		...
		$numservice = $data[3];
		$datesortie= $data[4]; 
		$query = "INSERT into Hospitalisation(NumPat,DateEntree,NumSalle,NumService,DateSortie) values('$numpat','$dateentree','$numsalle','$numservice','$datesortie')";
...
{
	$query = "DROP TABLE Hospitalisation";
...
<title>Database Query - Hospitalization</title>
</head>
<body>

<h1>Database Query - Hospitalization Table</h1>
