  ...
  if($filename[1] == 'csv')
  {
   ...
   while($data = fgetcsv($handle))
   {
	if ($i>0) {
		...
		$prenom = mysqli_real_escape_string($connect,$data[2]);
		$mutuelle = mysqli_real_escape_string($connect,$data[3]);
		$query = "INSERT into Patient(NumPat,Nom,Prenom,Mutuelle) values('$numpat','$nom','$prenom','$mutuelle')";
...
<title>Database Query - Patient</title>
</head>
<body>

<h1>Database Query - Patient Table</h1>
