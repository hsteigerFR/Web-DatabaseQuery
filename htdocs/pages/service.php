  ...
  if($filename[1] == 'csv')
  {
   ...
   while($data = fgetcsv($handle))
   {
	if ($i>0) {
		...
		$batiment = mysqli_real_escape_string($connect, $data[2]);
		$nummed = $data[3];
		$query = "INSERT into Service(NumService,Nom,Batiment,NumMed) values('$numservice','$nom','$batiment',$nummed)";
...
<title>Database Query - Department</title>
</head>
<body>

<h1>Database Query - Department Table</h1>
